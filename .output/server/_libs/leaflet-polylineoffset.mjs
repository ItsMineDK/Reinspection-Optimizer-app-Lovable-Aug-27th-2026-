...
//#region node_modules/leaflet-polylineoffset/leaflet.polylineoffset.js
var require_leaflet_polylineoffset = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(factory, window) {
		if (typeof define === "function" && define.amd) define(["leaflet"], factory);
		else if (typeof exports === "object") module.exports = factory(require_leaflet_src());
		if (typeof window !== "undefined" && window.L) window.L.PolylineOffset = factory(L);
	})(function(L) {
		function forEachPair(list, callback) {
			if (!list || list.length < 1) return;
			for (var i = 1, l = list.length; i < l; i++) callback(list[i - 1], list[i]);
		}
		/**
		...
		*/
		function lineEquation(pt1, pt2) {
			if (pt1.x === pt2.x) return pt1.y === pt2.y ? null : { x: pt1.x };
		...
		*/
		function intersection(l1a, l1b, l2a, l2b) {
			...
			if (line1.hasOwnProperty("x")) return line2.hasOwnProperty("x") ? null : {
				x: line1.x,
				y: line2.a * line1.x + line2.b
			};
			if (line2.hasOwnProperty("x")) return {
				x: line2.x,
		...
		var PolylineOffset = {
			offsetPointLine: function(points, distance) {
				...
				forEachPair(points, L.bind(function(a, b) {
					...
					var offsetAngle = Math.atan2(a.y - b.y, a.x - b.x) - Math.PI / 2;
					offsetSegments.push({
						offsetAngle,
						original: [a, b],
			...
			*/
			circularArc: function(s1, s2, distance) {
				if (s1.offsetAngle === s2.offsetAngle) return [s1.offset[1]];
				const signedAngle = this.getSignedAngle(s1.offset, s2.offset);
				if (signedAngle * distance > 0 && signedAngle * this.getSignedAngle(s1.offset, [s1.offset[0], s2.offset[1]]) > 0) return [intersection(s1.offset[0], s1.offset[1], s2.offset[0], s2.offset[1])];
				var points = [];
				...
				var step = Math.PI / 8;
				for (var alpha = startAngle; alpha < endAngle; alpha += step) points.push(translatePoint(center, distance, alpha));
				points.push(translatePoint(center, distance, endAngle));
				return rightOffset ? points.reverse() : points;
			}
		};
		L.Polyline.include({ _projectLatlngs: function(latlngs, result, projectedBounds) {
			if (latlngs.length > 0 && latlngs[0] instanceof L.LatLng) {
				...
				if (this.options.offset) ring = L.PolylineOffset.offsetPoints(ring, this.options.offset);
				result.push(ring.map(function(xy) {
					return L.point(xy.x, xy.y);
				}));
			} else latlngs.forEach(L.bind(function(ll) {
				this._projectLatlngs(ll, result, projectedBounds);
			}, this));
		} });
		L.Polyline.include({ setOffset: function(offset) {
...
//#endregion
export { require_leaflet_polylineoffset as t };
